...
- (void)viewDidLoad {
    ...
    // 获取网络缓存大小
    PPLog(@"网络缓存大小cache = %fKB",[ORNetworkCache getAllHttpCacheSize]/1024.f);
    ...
    NSString *num6 = [ORMethods return6Number];
    NSString *time = [ORMethods getNowTimeTimestamp];
    NSString *priceString = [ORMethods returnSignMethod:@"getCommendPrice" Rand:num6 Time:time];
    
    NSDictionary *priceDict = @{
                                @"method":@"getCommendPrice",
                                @"sign":[ORMethods MD5ForLower32Bate:priceString],
                                @"time":time,
                                @"rand":num6,
                                };
    
    [ORNetwork GET:@"http://192.168.100.40:8081/service/api" parameters:priceDict responseCache:^(id responseCache) {
        // 1.先加载缓存数据
...
- (void)getData:(BOOL)isOn url:(NSString *)url
{
    ...
    if(isOn)
    {
        ...
        [ORNetwork GET:url parameters:para responseCache:^(id responseCache) {
            // 1.先加载缓存数据
            self.cacheData.text = [self jsonToString:responseCache];
        } success:^(id responseObject) {
            // 2.再请求网络数据
    ...
    else
    {
        ...
        [ORNetwork GET:url parameters:para success:^(id responseObject) {
            self.networkData.text = [self jsonToString:responseObject];
        } failure:^(NSError *error) {
...
- (void)monitorNetworkStatus
{
    ...
    [ORNetwork networkStatusWithBlock:^(ORNetworkStatusType networkStatus) {
        
        switch (networkStatus) {
                // 未知网络
            case ORNetworkStatusUnknown:
                // 无网络
            case ORNetworkStatusNotReachable:
                self.networkData.text = @"没有网络";
                [self getData:YES url:dataUrl];
                PPLog(@"无网络,加载缓存数据");
                break;
                // 手机网络
            case ORNetworkStatusReachableViaWWAN:
                // 无线网络
            case ORNetworkStatusReachableViaWiFi:
...
- (IBAction)download:(UIButton *)sender {
    ...
    if(!self.isDownload)
    {
        ...
        task = [ORNetwork downloadWithURL:downloadUrl fileDir:@"Download" progress:^(NSProgress *progress) {
